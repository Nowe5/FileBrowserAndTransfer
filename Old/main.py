...
class ssh_file_manager():
    ...
    def drawPwdLs(self, stdscr): #orta menü
        
        start_x =   int((self.width // 2))
        start_y =   int((self.height // 2) - 2)
        maxCol  =   self.width // 3-3
        max_row =   self.height-5 #max number of rows
        ...
        boxChild = drawChild(stdscr, self.strings[0])
        ...
        statusPwd=self.ourPwd
        if statusPwd.startswith("/home/"+userName):
            statusPwd = statusPwd.replace("/home/"+userName, "~")
        stdscr.addstr(0, 2, userName+"@"+ hostName+" " +statusPwd+ "/" + self.strings[0])
        #TODO root ta /root ~ görünmüyor
        
        # Render Bottom Status bar
        stdscr.attron(curses.color_pair(3))
        stdscr.addstr(self.height-1, 0, statusbarstr)    #statusbarstr
        ...
        for i in range( 1, max_row  + 1 ):
            if self.row_num == 0:
                box.addstr( 1, 1, "There aren't strings", self.highlightText )
            else:
                if (i == position):
                    box.addstr( i, 2, str( i ) + " - " + strings[ i - 1 ], self.highlightText )
                else:
                    box.addstr( i, 2, str( i ) + " - " + strings[ i - 1 ], self.normalText )
                if i == row_num:
                    break
...
def main():
    try:
        curses.wrapper(ssh_file_manager)
